#!/usr/bin/env node
// Generate an Alpine Strike video without touching the app: drives the bowling
// page in headless Chrome and records the episode to a .webm file
// (YouTube-ready). The sibling of scripts/record-race.mjs, record-hatch.mjs and
// record-jump.mjs.
//
// Usage (dev server must be running — `npm run dev`):
//   npm run record:bowl -- [options]
//   node scripts/record-bowl.mjs --riders 4 --ride 60 --out strike.webm
//
// Options:
//   --riders <n>       How many animals ride (2–4)               (default 3)
//   --pick <a,b,c>     Specific animals by name, in order; overrides --riders
//   --ride <secs>      How long the leader's ride lasts, 15–150  (default 35)
//   --hazards <pct>    How thickly the mountain is littered, 0–100  (default 65)
//   --difficulty <n>   Bunny | Classic | Black                 (default Classic)
//   --together         Everyone rides at once (default: one at a time)
//   --loose            Turn "keep it close" off (only with --together)
//   --env <name>       Environment chip: Summer | Winter | …   (default Winter)
//   --weather <name>   Weather chip: Auto | None | Snow | …    (default Auto)
//   --out <file>       Output video path           (default strike-<time>.webm)
...
const pick = opt('pick', '')
  ...
  .map((s) => s.trim())
  .filter(Boolean)
const riders = Math.max(2, Math.min(4, parseInt(opt('riders', '3'), 10)))
const ride = Math.max(15, Math.min(150, parseInt(opt('ride', '35'), 10)))
const hazards = Math.max(0, Math.min(100, parseInt(opt('hazards', '65'), 10)))
const difficulty = opt('difficulty', 'Classic')
const env = opt('env', 'Winter')
const weather = opt('weather', 'Auto')
const out = opt('out', `strike-${Date.now()}.webm`)
const [w, h] = opt('size', '1280x720').split('x').map((n) => parseInt(n, 10))
const hold = parseFloat(opt('hold', '8'))
const url = opt('url', 'http://localhost:5173')

// Matches the beat timings in src/bowl/model.ts, plus room to breathe. The ride
// is open-ended and the mountain is only built to hit the asked-for time on
// average, so budget for it running well over, plus the smash window on top.
const rideMs = ride * 1000 * 2 + 12000
const turns = flag('together') ? 1 : pick.length || riders
const episodeMs = 4200 + 3200 + rideMs * turns + 16000 * turns + 6000 + 7000 + 20000

const videoDir = join(tmpdir(), `runkids-strike-${Date.now()}`)
mkdirSync(videoDir, { recursive: true })

console.log(
  `Recording: ${pick.length ? `animals=${pick.join('+')}` : `riders=${riders}`}` +
    ` ride=${ride}s hazards=${hazards}% difficulty=${difficulty}` +
...
try {
  await page.goto(`${url.replace(/#.*$/, '')}#/bowl`, { waitUntil: 'networkidle' })
  ...
  if (pick.length) {
    ...
    for (let i = 0; i < Math.min(riders, cards); i++) {
  ...
  await page
    .locator('.slider-line', { hasText: 'Average ride' })
    .locator('input')
    .fill(String(ride))
  await page
    .locator('.slider-line', { hasText: 'Hazards' })
    .locator('input')
    .fill(String(hazards))
  if (flag('together')) {
    await page.locator('.env-chip', { hasText: 'All together' }).first().click()
